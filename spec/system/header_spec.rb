...
RSpec.describe "ヘッダーのテスト", type: :request do
  describe 'ログインしていない場合' do
    ...
    context 'ヘッダーのリンクを確認' do
      ...
      it 'root画面に遷移する' do
        ...
        is_expected.to match(root_path)
      end
      it '予約画面に遷移する' do
        visit find_all('a')[1][:href]
        is_expected.to eq(reservations_path)
      end
      it 'お問い合わせ画面に遷移する' do
        visit find_all('a')[2][:href]
        is_expected.to eq(inquiry_path)
      end
      it '大きな文字メニュー画面を表示する' do
      ...
      it '診察のご案内画面に遷移する' do
        ...
        is_expected.to eq(exams_path)
      end
      it '交通案内画面に遷移する' do
        visit find_all('a')[6][:href]
        is_expected.to eq(access_path)
      end
      it '会員ページ画面に遷移する' do
        visit find_all('a')[7][:href]
        is_expected.to eq(new_patient_session_path)
      end
    end
  end
  ...
  describe '会員ログインしている場合' do
    ...
    context 'ヘッダーのリンクを確認' do
      ...
      it '予約確認画面に遷移する' do
        visit find_all('a')[3][:href]
        is_expected.to eq(patients_homes_reservation_path)
      end
      it '新規予約作成画面に遷移する' do
        visit find_all('a')[4][:href]
        is_expected.to eq(reservations_path)
  ...
  describe '管理者ログインしている場合' do
    ...
    context 'ヘッダーのリンクを確認' do
      ...
      it '管理者情報画面に遷移する' do
        visit find_all('a')[3][:href]
        is_expected.to eq(admins_about_path)
      end
      it '会員一覧画面に遷移する' do
        visit find_all('a')[4][:href]
        is_expected.to eq(admins_homes_patients_path)
      end
      it '本日の予約一覧画面に遷移する' do
        visit find_all('a')[5][:href]
        is_expected.to eq(admins_reservations_today_path)
      end
      it '予約一覧画面に遷移する' do
        visit find_all('a')[6][:href]
        is_expected.to eq(admins_reservations_all_path)
      end
    end
  end
end
